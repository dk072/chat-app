        ...
        <header className="flex flex-row items-center justify-between gap-2 bg-white p-3 sm:p-4 rounded-2xl sm:rounded-3xl border border-slate-100 shadow-xs">
          ...
          <div className="flex items-center space-x-2 shrink-0">
            <button
              ...
              className="px-3 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-xl text-xs font-bold flex items-center space-x-1.5 transition-all cursor-pointer active:scale-95 disabled:opacity-50"
              title="Refresh Dashboard Data"
            >
              <RefreshCw className={`w-3.5 h-3.5 text-slate-600 ${isRefreshingGlobal ? 'animate-spin text-brand-600' : ''}`} />
              <span className="hidden sm:inline">{isRefreshingGlobal ? 'Refreshing...' : 'Refresh'}</span>
            </button>

            <button
              onClick={() => setIsCopilotOpen(true)}
              className="px-3 sm:px-3.5 py-2 bg-indigo-50 text-indigo-700 hover:bg-indigo-100 rounded-xl text-xs font-bold flex items-center space-x-1.5 sm:space-x-2 transition-all"
            >
